...
*You could have loaded it with a simple drag & drop, but for the rest of the tutorial, you really need to go through the menus and the file request.*


## Load a ball as a brush

Go **Brush menu**, **Open** and select `blueball.iff`. It load a brush of a blue ball, what a sense of naming, really!
...
First select the **Dots** tool, first line on the left.![](../../../tools/src/t-dot.png)  
...
*Although PydPainter has a level of 25 undo, it cannot handle this on different frames. So if you want to redo your trajectory in the difficult art of animation (and ball bouncing), you're in trouble.*
...
Redo all steps from the beginning, *sorry:-(*.
...
3. Hold down the Animpainting key of your system.
...
8. Delete and start again. 

Move the ball around the screen, clicking frantically with the left mouse button as if it were the trajectory you wanted it to follow.

You can also try drawing by holding down the left mouse button all the way, but this will make clones of the ball.
...
*it's not compulsory, if you start from frame 25, the animpainting will loop and continue after 100 from 1 to 24.*
...
Go to the **Spacing** options of the Line tool (right-click on the upper-left part of its icon). 
...
![](7-linear-20.jpg)
1. Start from 1.
2. Hit **F10** thrice to hide the right toolbar.
2. Press the Animpainting key (remember `Left-Windows` key or the `Left-Command` key on the Mac).
3. Draw a line from the far left to the right of the screen. 
...
- the ball arrives from the left of the screen and falls like a balloon
...
Now that you've understood the principle, you're thinking that the ball should leave the screen a little faster, so the last part of the game should be made up of fewer frames (thus reducing the total N value). You're right, you've got it.
...
## Others drawing tools

Now that you've got the hang of it, you can get to grips with the remaining drawing tools:

![](../../../tools/src/t-circle.png) **Circle** Tool will make you brushs that draw circles.
